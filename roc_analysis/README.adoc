:source-highlighter: pygments

= Receiver Operating Characteristics (ROC)



== Event graph

ifndef::backend-pdf[]
++++
<iframe width="100%", height="600", src="event_graph.html" name="targetframe" allowTransparency="true" scrolling="no" frameborder="0"> </iframe>
++++
endif::[]
ifdef::backend-pdf[]
image::event_graph.png[]
endif::[]
ifdef::env-github[]
image: https://github.com/timcardenuto/ids-scripts/tree/master/roc_analysis/images/event_graph.png[]
endif::[]


== ROC curve

ifndef::backend-pdf[]
++++
<iframe width="100%", height="600", src="roc_graph.html" name="targetframe" allowTransparency="true" scrolling="no" frameborder="0"> </iframe>
++++
endif::[]
ifdef::backend-pdf[]
image::roc_graph.png[]
endif::[]
ifdef::env-github[]
image: https://github.com/timcardenuto/ids-scripts/tree/master/roc_analysis/images/roc_graph.png[]
endif::[]


== Precision-Recall curve

ifndef::backend-pdf[]
++++
<iframe width="100%", height="600", src="pr_graph.html" name="targetframe" allowTransparency="true" scrolling="no" frameborder="0"> </iframe>
++++
endif::[]
ifdef::backend-pdf[]
image::pr_graph.png[]
endif::[]
ifdef::env-github[]
image: https://github.com/timcardenuto/ids-scripts/tree/master/roc_analysis/images/pr_graph.png[]
endif::[]


== Accuracy curve

ifndef::backend-pdf[]
++++
<iframe width="100%", height="600", src="accuracy_graph.html" name="targetframe" allowTransparency="true" scrolling="no" frameborder="0"> </iframe>
++++
endif::[]
ifdef::backend-pdf[]
image::accuracy_graph.png[]
endif::[]
ifdef::env-github[]
image: https://github.com/timcardenuto/ids-scripts/tree/master/roc_analysis/images/accuracy_graph.png[]
endif::[]


